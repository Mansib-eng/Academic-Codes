...
void boundaryFill8(int x, int y, unsigned char fillCol[3], unsigned char boundaryCol[3])
{
    // Safety bounds
    ...
    if (!sameColor(current, boundaryCol) && !sameColor(current, fillCol))
    {
        ...
        boundaryFill8(x + 1, y, fillCol, boundaryCol);
        boundaryFill8(x - 1, y, fillCol, boundaryCol);
        boundaryFill8(x, y + 1, fillCol, boundaryCol);
        boundaryFill8(x, y - 1, fillCol, boundaryCol);

        // Diagonal neighbors (extra 4)
        boundaryFill8(x + 1, y + 1, fillCol, boundaryCol);
        boundaryFill8(x - 1, y + 1, fillCol, boundaryCol);
        boundaryFill8(x + 1, y - 1, fillCol, boundaryCol);
        boundaryFill8(x - 1, y - 1, fillCol, boundaryCol);
    }
...
void display()
{
    ...
    // ✅ Seed point inside the rectangle (auto fill)
    boundaryFill8(250, 250, fillColor, boundaryColor);
...
void mouse(int button, int state, int x, int y)
{
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
    {
        ...
        boundaryFill8(x, newY, fillColor, boundaryColor);
    }
}
...
void init()
{
    ...
    // Helps boundary color stay exact (recommended)
    glDisable(GL_DITHER);
